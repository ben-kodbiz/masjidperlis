...
Serve the `public/` folder with any static server, for example. The masjid and
event detail pages are generated by the site builder, so build them first:

```bash
python3 tools/build_site.py                       # generate detail pages into public/
...
python3 tools/serve.py                            # admin panel → :8000/admin/ (serves masjid/event pages too)
