...
def test_crud_masjid_speaker_category():
    ...
    try:
        status, created = admin.request("POST", "/api/masjids",
                                        {"name": "Masjid Ujian", "mukim": "Kangar"})
        assert status == 201, (status, created)
        mid = created["id"]
        assert mid == "masjid-ujian", mid
        # mukim_id derived from the free-text mukim
        assert created["record"]["mukim_id"] == "kangar", created
        ...
        status, _ = admin.request("PUT", f"/api/masjids/{mid}",
                                  {"name": "Masjid Ujian Baru", "mukim": "Arau"})
...
def test_event_lifecycle_and_status():
    admin = Admin()
    try:
        _, created = admin.request("POST", "/api/masjids", {"name": "Masjid Alwi", "mukim": "Kangar"})
        mid = created["id"]
...
def test_validation_rollback():
    admin = Admin()
    try:
        _, created = admin.request("POST", "/api/masjids", {"name": "Masjid Rollback", "mukim": "Arau"})
        mid = created["id"]
...
def test_publish_mirrors_data():
    admin = Admin()
    try:
        _, created = admin.request("POST", "/api/masjids", {"name": "Masjid Publish", "mukim": "Kangar"})
...
def test_mukim_editor_crud_and_references():
    admin = Admin()
    try:
        # new data dirs start with the official Perlis mukims
        mukims = admin.data()["mukims"]
        assert any(d["id"] == "kangar" for d in mukims), mukims
        ...
        # add a new mukim
        status, mukim = admin.request("POST", "/api/mukims",
                                         {"name": "Titi Tinggi", "description": "Baharu"})
        assert status == 201, (status, mukim)
        did = mukim["id"]
        assert did == "titi-tinggi", did

        # masjid linking mukim_id + editor_id
        ...
        mid = masjid["id"]
        assert masjid["record"]["mukim_id"] == did
        assert masjid["record"]["mukim"] == "Titi Tinggi", masjid["record"]
        assert masjid["record"]["editor_id"] == eid

        # updating the mukim name without changing mukim_id => mismatch must reject
        status, resp = admin.request("PUT", f"/api/mukims/{did}",
                                     {"name": "Titi Tinggi Lama", "description": "Baharu"})
        assert status == 400, (status, resp)
        assert any("does not match" in e for e in resp["errors"]), resp

        # delete the referenced mukim -> blocked
        status, resp = admin.request("DELETE", f"/api/mukims/{did}")
        assert status == 400, (status, resp)
        ...
        status, _ = admin.request("DELETE", f"/api/masjids/{mid}")
        assert status == 200, status
        assert admin.request("DELETE", f"/api/mukims/{did}")[0] == 200
...
def test_static_pages_served():
    ...
    try:
        for page in ("/admin/index.html", "/admin/events.html", "/admin/event-editor.html",
                     ...
                     "/admin/mukims.html", "/admin/editors.html",
